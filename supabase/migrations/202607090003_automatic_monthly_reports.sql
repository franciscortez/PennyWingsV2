...
begin
  ...
  /*
   * Account tables store only today's balance. Rewind transactions dated on or
   * after the next month to reconstruct the selected month's closing balance.
   * This keeps a June report historically correct even when first viewed in July.
   */
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
      ...
      card.balance - coalesce((
        select sum(
          case
            ...
            when t.type in ('expense', 'withdrawal') and t.card_id = card.id
              then -t.amount
            else 0
          end
          + case
              when t.type = 'transfer' and t.card_id = card.id then -t.amount
              else 0
            end
          + case
              when t.type = 'transfer' and t.to_card_id = card.id then t.amount
              else 0
            end
        )
        from public.transactions as t
      ...
      wallet.balance - coalesce((
        select sum(
          case
            ...
            when t.type in ('expense', 'withdrawal') and t.wallet_id = wallet.id
              then -t.amount
            else 0
          end
          + case
              when t.type = 'transfer' and t.wallet_id = wallet.id then -t.amount
              else 0
            end
          + case
              when t.type = 'transfer' and t.to_wallet_id = wallet.id then t.amount
              else 0
            end
          + case
