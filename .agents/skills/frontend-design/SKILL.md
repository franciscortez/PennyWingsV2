...
Source: [Anthropic frontend-design plugin](https://github.com/anthropics/claude-code/tree/main/plugins/frontend-design), version 1.1.0. This project-local skill makes its design guidance available to Codex without requiring Claude.

For PennyWings, pair this visual design guidance with `$implement-frontend-feature` and `$verify-pennywings-change`. Preserve the requested screen sizes and existing product behavior; do not add dependencies or change database contracts for a visual redesign.
